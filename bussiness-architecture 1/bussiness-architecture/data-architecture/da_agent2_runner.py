...
PROMPT_FILE = Path(__file__).parent / "DA_REVIEW_PROMPT.md"
...
def _call_claude(prompt_content: str, timeout: int = 600):
    ...
    try:
        ...
        print("  [warn] claude CLI timed out after 600s")
        return None
...
def run_claude_agent(prompt: str, task_file: str, input_dir: str) -> bool:
    ...
    while not review_file.exists() and attempt < _MAX_RETRIES:
        ...
        call = _call_claude(_build_retry_prompt(task_file, da_output_dir))
        if call is None:
            break
        returncode, stdout, stderr = call
        if returncode != 0:

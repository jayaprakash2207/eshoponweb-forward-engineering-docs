...
Validation per GR-7. Before completion:
  ...
  - Per declared output (system-inventory-pack, module-boundary-pack, component-registry-pack, dependency-pack, entry-point-pack, call-flow-pack, layering-pattern-pack, external-boundary-pack, frontend-application-pack): confirm it exists, is well-formed, and is traceable.
...
Escalation: an ASSUMED on a material node (module-boundary, component, call-flow) → raise Open Question and block HIGH-only

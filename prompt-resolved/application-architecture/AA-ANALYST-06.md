...
Validation per GR-7. Before completion:
  ...
  - Per declared output (application-security-assessment): confirm it exists, is well-formed, and is traceable.
...
Escalation: an ASSUMED on a material node (security-control, endpoint-exposure) → raise Open Question and block HIGH-only

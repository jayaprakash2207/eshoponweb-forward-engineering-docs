...
Validation per GR-7. Before completion:
  ...
  - Per declared output (symbol-registry, route-registry, dependency-candidates, entry-point-candidates): confirm it exists, is well-formed, and is traceable.
...
Escalation: an ASSUMED on a material node (symbol, route, entry-point) → raise Open Question and block HIGH-only

...
Validation per GR-7. Before completion:
  ...
  - Per declared output (quality-review, executive-summary-for-review, final-sanity-check): confirm it exists, is well-formed, and is traceable.
...
Escalation: an ASSUMED on a material node (component, dependency, violation) → raise Open Question and block HIGH-only

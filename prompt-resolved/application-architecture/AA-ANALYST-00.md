...
Validation per GR-7. Before completion:
  ...
  - Per declared output (full-application-architecture-package): confirm it exists, is well-formed, and is traceable.
...
Escalation: an ASSUMED on a material node (component, interface, dependency, architecture-pattern) → raise Open Question and block HIGH-only

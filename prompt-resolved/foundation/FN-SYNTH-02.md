...
Validation per GR-7. Before completion:
  ...
  - Per declared output (canonical-enterprise-model, architecture-inventory, traceability-matrix, forward-engineering-input-map): confirm it exists, is well-formed, and is traceable.
...
Escalation: an ASSUMED on a material node (capability, entity, service, api) → raise Open Question and block HIGH-only
